...
<div class="card">
    ...
    <p class="muted">
        Enviamos un enlace de verificacion a <strong>{{ $registration->owner_email }}</strong>. La clinica se creara cuando confirmes ese correo.
    </p>
    ...
    @endphp
    <span class="pill {{ $statusClass }}">{{ str_replace('_', ' ', $registration->status) }}</span>

    <div class="box">
        <p><strong>Clinica:</strong> {{ $registration->nombre_centro }}</p>
        <p><strong>Enlace vence:</strong> {{ optional($registration->verification_expires_at)->format('d/m/Y H:i') ?? 'Sin fecha' }}</p>
    ...
    <div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap;">
        @if($registration->isProvisioned() && $registration->onboarding_redirect_url)
            <a class="btn-secondary" href="{{ $registration->onboarding_redirect_url }}">Continuar al onboarding</a>
        @elseif($canResend)
